...
graph TB
    subgraph 外部系统
        FORUM[Discourse 论坛]
        GITCODE[GitCode 文档仓库]
        SILICONFLOW[SiliconFlow API<br/>LLM/Embedding/Rerank]
        LIGHTRAG[LightRAG 服务]
    end

    subgraph 数据准备层 L1
        FS[ForumSource<br/>论坛数据获取]
        GS[GitCodeSource<br/>文档获取]
        CK[Chunker<br/>文档分块]
        EMB[EmbeddingClient<br/>向量化]
        ING[PgVectorIngestor<br/>索引写入]
        DW[DualWriter<br/>双写链路]
    end

    subgraph 检索层 L2
        VR[VectorRetriever<br/>向量召回]
        BR[BM25Retriever<br/>全文检索]
        LR[LightRAGRetriever<br/>知识图谱检索]
        QR[QueryRewriter<br/>Multi-Query改写]
    end

    subgraph 处理层 L3
        RRF[RRFFusion<br/>RRF融合+去重]
        RK[Reranker<br/>BGE重排]
    end

    subgraph 生成层 L4
        PB[PromptBuilder<br/>Prompt组装]
        LLM[LLMClient<br/>大模型调用]
    end

    subgraph 编排与观测
        PIPE[RagPipeline<br/>四层编排]
        TRACE[Tracing<br/>结构化日志]
        EVAL[EvalFramework<br/>测评框架]
    end

    subgraph 数据存储
        PG[(PostgreSQL<br/>pgvector + BM25)]
        JSONL[logs/trace.jsonl]
    end
...
sequenceDiagram
    ...
    M->>FC: 新帖子 topic
    FC->>T: TraceContext.new(topic_id)
    FC->>QR: rewrite(query)
    QR->>SF: LLM 改写
    SF-->>QR: 3 路子查询
    
    par 多路检索
        FC->>VR: retrieve_multi(queries, top_k=50)
        VR->>SF: embed(query)
        VR->>PG: SELECT ... ORDER BY embedding <=> qvec
        PG-->>VR: vector hits
    and
        FC->>BR: retrieve_multi(queries, top_k=50)
        BR->>PG: SELECT ... WHERE content_tsv @@ tsq
        PG-->>BR: BM25 hits
    end

    FC->>RRF: process(all_hits, top_k=10)
    RRF-->>FC: 融合+去重后 top-10
    FC->>RK: process(hits, top_k=10)
    RK->>SF: POST /rerank
    SF-->>RK: relevance_scores
    RK-->>FC: 重排后 top-10

    FC->>PB: build(question, context)
    FC->>LLM: generate(question, context)
    LLM->>SF: POST /chat/completions
    SF-->>LLM: answer
    LLM-->>FC: GenerationResult

    FC->>T: trace_event(retrieval_ms, processing_ms, generation_ms)
    FC->>PG: INSERT request_traces
    FC-->>M: 最终回答
```
