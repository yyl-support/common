...
from .SchemaValidation.end_to_end_check import run_schema_check
...
class ForumMonitor:
    ...
    def _process_pre_audit_topic(self, topic):
        ...
        try:
            ...
            # 无评审结果时不生成回复（例如帖子虽含关键字但无 Redfish 相关评审点）
            if not answer or not answer.strip():
                logger.info(f"[预审] 帖子 {topic_id} 无 Redfish 相关评审点，跳过回复")
                self.data_processor.append_to_db([topic], 'pre_audit_processed_topics')
                return

            # 检查回复质量：只要有任何规则检查结果为"空响应"，视为无效回复，不发送到论坛
            if '空响应' in answer or 'Empty response' in answer:
                empty_response_count = answer.count('空响应') + answer.count('Empty response')
                logger.warning(f"[预审] 帖子 {topic_id} 的评审结果中包含 {empty_response_count} 处'空响应'，判定为无效回复，跳过发送")
                self.data_processor.append_to_db([topic], 'pre_audit_processed_topics')
                return
