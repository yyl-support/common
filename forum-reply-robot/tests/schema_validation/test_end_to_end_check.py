...
def test_process_post_non_redfish_short_circuits(monkeypatch):
    monkeypatch.setattr(e2e, "is_post_redfish_related", lambda title, content: (False, "not redfish"))
...
def test_process_post_redfish_success_flow(monkeypatch):
    ...
    monkeypatch.setattr(e2e, "is_post_redfish_related", lambda title, content: (True, "has redfish"))
    monkeypatch.setattr(e2e, "extract_review_points", lambda content: [{"title": "rp1", "content": "c1"}])
    monkeypatch.setattr(e2e, "filter_redfish_review_points", lambda rps: (rps, []))
    monkeypatch.setattr(
        e2e,
...
def test_append_error_details_writes_all_sections():
    ...
    e2e._append_error_details(
        lines,
        {
            "STATIC_VALIDATION": [{"rule": "Schema", "message": "bad", "advice": "fix schema"}],
            "MODEL_VALIDATION": [{"rule": "Rule-1", "message": "fail", "advice": "fix rule"}],
            "WARNING_DETAILS": [{"rule": "Rule-2", "message": "warn", "advice": "review"}],
        },
    )
...
def test_process_post_returns_empty_when_no_review_points(monkeypatch):
    monkeypatch.setattr(e2e, "is_post_redfish_related", lambda title, content: (True, "redfish"))
    monkeypatch.setattr(e2e, "extract_review_points", lambda content: [])
...
def test_process_post_returns_empty_when_no_redfish_points(monkeypatch):
    monkeypatch.setattr(e2e, "is_post_redfish_related", lambda title, content: (True, "redfish"))
    monkeypatch.setattr(e2e, "extract_review_points", lambda content: [{"title": "rp", "content": "plain"}])
    monkeypatch.setattr(e2e, "filter_redfish_review_points", lambda points: ([], points))
