...
class TestForumClient:
    ...
    def test_retrieve_documents_uses_rag_backend(self, client):
        """测试 forum_client 是通过 RagBackend 接口取检索结果"""
        fake = Mock()
        fake.query.return_value = {'related_docs': 'X', 'data': {'chunks': []}}
        client.rag_backend = fake

        result = client.retrieve_documents_for_topic(
            {'id': 7, 'title': 't', 'user_question': 'q'}
        )

        fake.query.assert_called_once()
        assert result == {'topic_id': 7, 'related_docs': 'X', 'data': {'chunks': []}}